...
export default function BuildingMaintenancePage() {
  ...
  const service = STITCH_SERVICES.find((s) => s.id === 'building-maintenance')!;
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <div className="max-w-3xl space-y-4">
            <span className="text-xs font-semibold uppercase tracking-widest text-[#C85A32]">
              {isAr ? 'قطاع هندسي معتمد • 02' : 'VERIFIED DISCIPLINE • 02'}
            </span>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            <div className="lg:col-span-7 space-y-6">
              <span className="text-xs font-semibold uppercase tracking-widest text-[#C85A32]">
                {isAr ? 'برامج الصيانة الوقائية' : 'PREVENTATIVE ASSET UPKEEP'}
              </span>
              <h2 className="font-serif text-3xl sm:text-4xl font-normal text-[#181A1B] dark:text-[#F7F5F0]">
                {isAr ? 'استمرارية التشغيل وحماية الأصول' : 'Operational Reliability & Asset Preservation'}
              </h2>
              ...
              <div className="pt-4 border-t border-[#E2DED6] dark:border-[#2C2F33] space-y-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-[#C85A32] block">
                  {isAr ? 'خدمات الصيانة المعتمدة:' : 'Maintenance Capabilities:'}
                </span>
